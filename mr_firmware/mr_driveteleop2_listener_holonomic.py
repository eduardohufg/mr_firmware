...
class DTeleopListener(Node):
    ...
    def swerve_mover(self,data):
        if self.buttons[1]:
            if (self.servo2.getPhysicalPos == 120 and self.servo4.getPhysicalPos == 145 and self.servo6.getPhysicalPos == 165 and self.servo8.getPhysicalPos == 180):
                self.state = 0
                print("No holonomic mode")
        if self.buttons[2]:
            if (self.servo2.getPhysicalPos == 120 and self.servo4.getPhysicalPos == 145 and self.servo6.getPhysicalPos == 165 and self.servo8.getPhysicalPos == 180):
                self.state = 1
                print("Holonomic mode")
        if self.state == 0:
            ...
            self.servo5.motorMode(int((data.linear.x)*1000))
            self.servo7.motorMode(int((data.linear.x)*1000))
            self.servo2.moveTimeWrite(120+data.linear.y*45)
            self.servo4.moveTimeWrite(145+data.linear.y*45)
            self.servo6.moveTimeWrite(165+data.linear.y*45)
            self.servo8.moveTimeWrite(180+data.linear.y*45)
